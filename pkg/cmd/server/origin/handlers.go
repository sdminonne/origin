...
func (c *MasterConfig) impersonationFilter(handler http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if v, ok := req.Header[authenticationapi.ImpersonateUserScopeHeader]; ok {
			requestScopes = v
		}
		newUser, attributes, err := authorizer.ComputeActingUser(subjects, groupsSpecified, requestScopes, ctx, c.GroupCache, c.Authorizer)
		if err != nil {
			forbidden(err.Error(), attributes, w, req)
			return
		}
